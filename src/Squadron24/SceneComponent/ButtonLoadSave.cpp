...
ButtonLoadSave::ButtonLoadSave(float x, float y, float width, float height, const std::string& textureFile, std::string text, unsigned int police, std::string Font, int R, int G, int B) : Button(x, y, width, height, textureFile, text, police, Font, R, G, B)
{
    m_text.setPosition(x + 150, y);

}
void ButtonLoadSave::Update(sf::Time deltaTime, const sf::RenderWindow& window)
{
    sf::Mouse mouse;
